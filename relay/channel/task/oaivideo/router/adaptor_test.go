...
func TestRouterAdaptor_DelegateFor(t *testing.T) {
	...
	cases := []struct {
		...
		{"cy-sd7-seedance-2.0-1080p", "seedance-2.0", "seedance-magica"},
	...
	for _, tc := range cases {
		...
		case "seedance-magica":
			if _, ok := d.(*seedancemagica.TaskAdaptor); !ok {
				t.Fatalf("%s: expected seedance-magica adaptor", tc.origin)
			}
		case "seedance-huabu":
			if _, ok := d.(*seedancehuabu.TaskAdaptor); !ok {
				t.Fatalf("%s: expected seedance-huabu adaptor", tc.origin)
			}
