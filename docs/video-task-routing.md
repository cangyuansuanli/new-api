...
Leonardo `cy-sd4-*` vendor 在出站前严格校验参考素材数量：最多 4 张参考图、3 段参考视频、1 段参考音频。图片公开别名归一化、去空和去重后统一计数；超限直接返回 HTTP 400 与对应 `*_limit_exceeded` code，不创建任务、不预扣费，也不把请求发送到 Leonardo 插件。
