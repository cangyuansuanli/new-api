...
| `cy-sd8-seedance-2.0` → `sd2.0-933`；`cy-sd8-seedance-2.0-fast` → `sd-2.0-fast-v1` | seedance-huabu | 精确 internal/upstream 配对；卡脸版 9 图+3 视频+3 音频（参考图须遮眼）；快速版仅 9 图；`aspect_ratio`→`size`，参考图→上游 `image`（数组） | `/v1/videos/{id}` + `result_url` + 鉴权 `/content` |
