...
客户可见的图像 API 文档见 [`unified-image-api.md`](unified-image-api.md)。音乐/音频异步见 [`unified-audio-api.md`](unified-audio-api.md)。legacy chat image 转换仅用于读取和迁移历史客户端请求，不是模型广场或新客户端的公开调用方式。
