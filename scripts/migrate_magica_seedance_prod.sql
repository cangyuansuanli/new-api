...
(
    ...
    '{}', NULL,
    '{"images":5,"videos":3,"audios":3,"total":11,"fullReferenceMode":{"label":"多参参考","descriptionWithImages":"最多 5 图、3 视频、3 音频；带参考素材时 prompt 用 @Image1 或「图片1」"},"validationHint":"固定 720p；内部 480p 生成 + 拉高清。最多 5 张参考图、3 段参考视频、3 段参考音频。","showTempMediaHint":true,"prependReferenceGuide":true}',
    ...
    '[]',
    '[{"text":"固定 720p，按条计费；内部 480p 生成 + Video Upscaler，省积分。"},{"text":"支持文生、单图 i2v、多参参考（5 图 / 3 视频 / 3 音频）。"}]',
    EXTRACT(EPOCH FROM NOW())::BIGINT, EXTRACT(EPOCH FROM NOW())::BIGINT
),
(
    'video', 'video-tpl-magica-seedance-1080p-async', 'videos-json-async', 'seedance-flat', FALSE,
    '{}', NULL,
    '{"images":5,"videos":3,"audios":3,"total":11,"fullReferenceMode":{"label":"多参参考","descriptionWithImages":"最多 5 图、3 视频、3 音频；带参考素材时 prompt 用 @Image1 或「图片1」"},"validationHint":"固定 1080p；内部 480p 生成 + 拉高清。最多 5 张参考图、3 段参考视频、3 段参考音频。","showTempMediaHint":true,"prependReferenceGuide":true}',
    ...
    '[]',
    '[{"text":"固定 1080p，按秒计费；内部 480p 生成 + Video Upscaler，省积分。"},{"text":"支持文生、单图 i2v、多参参考（5 图 / 3 视频 / 3 音频）。"}]',
    EXTRACT(EPOCH FROM NOW())::BIGINT, EXTRACT(EPOCH FROM NOW())::BIGINT
)
...
SELECT
    ...
    EXTRACT(EPOCH FROM NOW())::BIGINT, EXTRACT(EPOCH FROM NOW())::BIGINT
FROM (VALUES
    ('cy-sd7-seedance-2.0-720p', 'Seedance 2.0 720p（Magica）：固定 720p，按条计费；文生/图生/多参参考，内部 480p+拉高清。', 'video-tpl-magica-seedance-720p-async'),
    ('cy-sd7-seedance-2.0-1080p', 'Seedance 2.0 1080p（Magica）：固定 1080p，按秒计费；文生/图生/多参参考，内部 480p+拉高清。', 'video-tpl-magica-seedance-1080p-async')
