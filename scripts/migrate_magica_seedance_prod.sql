...
(
    ...
    '{}', NULL,
    '{"images":5,"videos":3,"audios":3,"total":11,"fullReferenceMode":{"label":"多参参考","descriptionWithImages":"最多 5 图、3 视频、3 音频；prompt 可用 @Image1 引用"},"validationHint":"固定 1080p；最多 5 张参考图、3 段参考视频、3 段参考音频；不支持首尾帧。","showTempMediaHint":true,"prependReferenceGuide":true}',
...
SELECT
    ...
    EXTRACT(EPOCH FROM NOW())::BIGINT, EXTRACT(EPOCH FROM NOW())::BIGINT
FROM (VALUES
    ('cy-sd7-seedance-2.0-720p', 'Seedance 2.0 720p：固定 720p，按条计费；支持文生、图生、多参参考。', 'video-tpl-magica-seedance-720p-async'),
    ('cy-sd7-seedance-2.0-1080p', 'Seedance 2.0 1080p：固定 1080p，按条计费；支持文生、图生、多参参考。', 'video-tpl-magica-seedance-1080p-async')
